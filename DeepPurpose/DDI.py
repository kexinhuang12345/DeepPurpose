...
class DDI_Model:
    ...
    def test_(self, data_generator, model, repurposing_mode=False, test=False):
        ...
        if self.binary:
            ...
            return mean_squared_error(y_label, y_pred), pearsonr(y_label, y_pred)[0], pearsonr(y_label, y_pred)[
                1], concordance_index(y_label, y_pred), y_pred
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        training_generator = data.DataLoader(
            data_process_DDI_loader(train.index.values, train.Label.values, train, **self.config), **params)
        validation_generator = data.DataLoader(
            data_process_DDI_loader(val.index.values, val.Label.values, val, **self.config), **params)
    ...
    def predict(self, df_data):
        ...
        if (self.drug_encoding == "MPNN"):
            params['collate_fn'] = mpnn_collate_func
        elif self.drug_encoding in ['DGL_GCN', 'DGL_GAT', 'DGL_NeuralFP', 'DGL_GIN_AttrMasking', 'DGL_GIN_ContextPred',
                                    'DGL_AttentiveFP']:
            params_test['collate_fn'] = dgl_collate_func
