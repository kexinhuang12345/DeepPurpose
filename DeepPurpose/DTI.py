...
class Classifier(nn.Sequential):
    def __init__(self, model_drug, model_protein, **config):
        ...
        dims = [
                   self.input_dim_drug + self.input_dim_protein] + self.hidden_dims + [
                   1]
...
def repurpose(X_repurpose, target, model, drug_names=None, target_name=None,
              result_folder="./result/", convert_y=False, output_num_max=10,
              verbose=True):
    ...
    with open(fo, 'w') as fout:
        ...
        if model.binary:
            ...
            ### regression
            table_header = ["Rank", "Drug Name", "Target Name",
                            "Binding Score"]
            ...
            for i in range(len(X_repurpose)):
                if model.binary:
                    ...
                    string = 'Drug ' + '{:<{f_d}}'.format(drug_names[i],
                                                          f_d=f_d) + \
                             ' predicted to have binding affinity score ' + "{0:.2f}".format(
                        y_pred[i])
...
def virtual_screening(X_repurpose, target, model, drug_names=None,
                      target_names=None,
                      result_folder="./result/", convert_y=False,
                      output_num_max=10, verbose=True):
    # X_repurpose: a list of SMILES string
    ...
    with open(fo, 'w') as fout:
        ...
        if drug_names is not None and target_names is not None:
            ...
            for i in range(len(target)):
                if model.binary:
                    if y_pred[i] > 0.5:
                        string_lst = [drug_names[i], target_names[i], "YES",
                                      "{0:.2f}".format(y_pred[i])]

                    else:
                        string_lst = [drug_names[i], target_names[i], "NO",
                                      "{0:.2f}".format(y_pred[i])]

                else:
                    ### regression
                    string_lst = [drug_names[i], target_names[i],
                                  "{0:.2f}".format(y_pred[i])]
...
class DBTA:
    ...
    def __init__(self, **config):
        ...
        if drug_encoding == 'Morgan' or drug_encoding == 'ErG' or drug_encoding == 'Pubchem' or drug_encoding == 'Daylight' or drug_encoding == 'rdkit_2d_normalized' or drug_encoding == 'ESPF':
            ...
            self.model_drug = MLP(config['input_dim_drug'],
            ...
            self.model_drug = MPNN(config['hidden_dim_drug'],
                                   config['mpnn_depth'])
        else:
            raise AttributeError(
                'Please use one of the available encoding method.')
        ...
        if target_encoding == 'AAC' or target_encoding == 'PseudoAAC' or target_encoding == 'Conjoint_triad' or target_encoding == 'Quasi-seq' or target_encoding == 'ESPF':
            self.model_protein = MLP(config['input_dim_protein'],
    ...
    def test_(self, data_generator, model, repurposing_mode=False, test=False):
        ...
        for i, (v_d, v_p, label) in enumerate(data_generator):
            ...
            if self.binary:
                ...
                loss = loss_fct(n, Variable(
                    torch.from_numpy(np.array(label)).float()).to(self.device))
        ...
        if self.binary:
            ...
            return roc_auc_score(y_label, y_pred), average_precision_score(
                y_label, y_pred), f1_score(y_label, outputs), log_loss(y_label,
                                                                       outputs), y_pred
        else:
            if repurposing_mode:
                return y_pred
            return mean_squared_error(y_label, y_pred), \
                   pearsonr(y_label, y_pred)[0], pearsonr(y_label, y_pred)[
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        if test is not None:
            info = data_process_loader(test.index.values, test.Label.values,
                                       test, **self.config)
            ...
            testing_generator = data.DataLoader(
                data_process_loader(test.index.values, test.Label.values, test,
                                    **self.config), **params_test)
        ...
        if self.binary:
            ...
            valid_metric_header.extend(
                ["MSE", "Pearson Correlation", "with p-value",
                 "Concordance Index"])
        ...
        for epo in range(train_epoch):
            ...
            train = create_negatives_pdbind_sampling(positives_train)
            ...
            for i, (v_d, v_p, label) in enumerate(training_generator):
                ...
                if verbose:
                    if (i % 100 == 0):
                        t_now = time()
                        print('Training at Epoch ' + str(
                            epo + 1) + ' iteration ' + str(i) + \
                              ' with loss ' + str(loss.cpu().detach().numpy())[
                                              :7] + \
                              ". Total time " + str(
                            int(t_now - t_start) / 3600)[:7] + " hours")
            ...
            with torch.set_grad_enabled(False):
                if self.binary:
                    ...
                    if auc > max_auc:
                        ...
                        max_auc = auc
                    if verbose:
                        print('Validation at Epoch ' + str(
                            epo + 1) + ', AUROC: ' + str(auc)[:7] + \
                              ' , AUPRC: ' + str(auprc)[:7] + ' , F1: ' + str(
                            f1)[:7] + ' , Cross-entropy Loss: ' + \
                              str(loss)[:7])
                else:
                    ...
                    if verbose:
                        print('Validation at Epoch ' + str(
                            epo + 1) + ' with loss:' + str(loss_val.item())[
                                                       :7] + ', MSE: ' + str(
                            mse)[:7] + ' , Pearson Correlation: ' \
                              + str(r2)[:7] + ' with p-value: ' + str(p_val)[
                                                                  :7] + ' , Concordance Index: ' + str(
                            CI)[:7])
                        ...
                        writer.add_scalar("Loss/valid", loss_val.item(),
                                          iteration_loss)
            table.add_row(lst)
        ...
        if test is not None:
            ...
            if self.binary:
                auc, auprc, f1, loss, logits = self.test_(testing_generator,
                                                          model_max, test=True)
                ...
                if verbose:
                    print('Testing MSE: ' + str(
                        mse) + ' , Pearson Correlation: ' + str(r2)
                          + ' with p-value: ' + str(
                        p_val) + ' , Concordance Index: ' + str(CI))
            np.save(os.path.join(self.result_folder,
                                 str(self.drug_encoding) + '_' + str(
    ...
    def predict(self, df_data):
        ...
        '''
        print('predicting...')
        info = data_process_loader(df_data.index.values, df_data.Label.values,
                                   df_data, **self.config)
...
def create_negatives_pdbind_sampling(df, negative_sampling_fraction=1,
    ...
    random_ligands = ligand_df.sample(
        ...
        drop=True)
    replace_targets = False if len(random_ligands) <= len(protein_df)\
        else True
    random_targets = protein_df.sample(
        n=len(random_ligands),
        replace=replace_targets,
        random_state=random_state).reset_index(
        drop=True)
    negatives_df = pd.concat([random_ligands, random_targets], axis=1)
